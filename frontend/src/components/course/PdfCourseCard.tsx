...
export function PdfCourseCard({ row, busyAction, onDownload, onView, onGenerate }: PdfCourseCardProps) {
    ...
    <article className="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-muted-foreground">Course ID: {row.courseId}</span>
        </div>

        <h3 className="line-clamp-1 text-lg font-bold text-foreground">{row.courseTitle}</h3>
        ...
        <div className="rounded-lg border border-border bg-muted/40 p-2.5 text-xs">
          {hasPdf ? (
            ...
            <p className="text-muted-foreground">មិនទាន់មាន PDF សម្រាប់វគ្គនេះ</p>
          )}
        </div>
